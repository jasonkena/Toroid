...
\providecommand{\superToroid}[3]{
    \draw[shift={(-0.5,0.5)}] (1,-1) grid (#1+1,-#1-1);
        \foreach \x in {1,...,#1} {
            \foreach \y in {1,...,#1} {
        \pgfmathtruncatemacro{\xpos}{(\x-1)*#1+(\y-1)}
        \pgfmathtruncatemacro{\ypos}{((#3-1)*#1+(#2-1))}
        \ifnum\xpos<\ypos
        \providecommand{\param}{white}
        \else
        \providecommand{\param}{gray}
        \fi
\node[square,draw=black,fill=\param] at (\x,-\y)[align=center] {\AlphAlph{#2}\AlphAlph{#3}\\ \AlphAlph{\y}\AlphAlph{\x}};}}
}
...
\providecommand{\nestedToroid}[1]{
    \begin{tikzpicture}[scale=#1+1,square/.style={rectangle,inner sep=0pt,minimum size=1cm}]
        \begin{scope}[scale=1]
            \draw[shift={(-0.5,0.5)}] (1,-1) grid (#1+1,-#1-1);
            \foreach \x in {1,...,#1} {
            ...
            \foreach \xa in {1,...,#1} {
                \foreach \ya in {1,...,#1} {
                    \begin{scope}[shift={({\xa-0.5+1/(#1+1)^2},{-\ya+0.5-1/(#1+1)^2})},scale={1/(#1+1)}]
                        \toroidAxes{#1}{\xa}{\ya}
                    \end{scope}
                }
            }
...
%a
\providecommand{\twodcomparison}[1]{
\begin{tikzpicture}
    \begin{scope}[scale=1,square/.style={rectangle,inner sep=0pt,minimum size=1cm}]
        %\draw[shift={(-0.5,0.5)}] (0,0) grid ({(#1^2)},{-(#1^2)});
        \foreach \x in {1,...,#1} {
            \foreach \xa in {1,...,#1} {
